...
object DomainToXML:
  ...
  def generateOutputXML(outputSchedule: Result[OutputSchedule]): Elem =
  ...
  private def vivaListToXML(schedule: OutputSchedule): Elem =
    <schedule
            xsi:noNamespaceSchemaLocation="../../schedule.xsd" totalPreference={schedule.totalPreference.toString} xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
        {
          schedule.vivasSchedule.map( viva =>
